...
use crate::macros::error_to_unit_type;
	...
	pub fn parse(obj_file: &str) -> Result<Box<[Self]>, ()> {
		...
		for line in obj_file.lines().map(|str| str.trim()) {
			...
			if line.starts_with("v ") {
				...
				position_data.push([
					*line.get(0).ok_or(())?.as_ref().map_err(error_to_unit_type!())?,
					*line.get(1).ok_or(())?.as_ref().map_err(error_to_unit_type!())?,
					*line.get(2).ok_or(())?.as_ref().map_err(error_to_unit_type!())?,
				...
				while vertices.len() > 2 {
					macro_rules! extract_vertex {
						($vertex: expr) => {
							{
								let vertex = (
									*$vertex.get(0).ok_or(())?.as_ref().map_err(error_to_unit_type!())?,
