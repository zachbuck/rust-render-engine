	...
	pub fn get_variable_layout(binary: &[u32]) -> (Box<[DataType]>, Box<[DataType]>, Box<[DescriptorSet]>) {
		...
		while !(iter.is_at_end()) {
			...
			match instruction {
				...
				Instruction::OpTypeVoid			=> id_values[data[1] as usize] = Some(ID::DataType(DataType::Void)),
				...
				Instruction::OpTypeImage		=> id_values[data[1] as usize] = Some({
													let data_type = match &id_values[data[2] as usize] {
														Some(ID::DataType(DataType::Void))		=> Some(DataType::Void),
														Some(ID::DataType(DataType::Int)) 		=> Some(DataType::Int),
														Some(ID::DataType(DataType::UInt)) 		=> Some(DataType::UInt),
														Some(ID::DataType(DataType::Float)) 	=> Some(DataType::Float),
														Some(ID::DataType(DataType::Double)) 	=> Some(DataType::Double),
														_ 										=> None,
													};
													...
													if data_type.is_none() {
														...
														ID::DataType(DataType::Image { 
															dimension: Dim::from(data[3]),
															pixel_format: Box::new(data_type.unwrap()),
															texture_format: ImageFormat::from(data[8])
														})
													}
												}),
				Instruction::OpTypeSampler		=> id_values[data[1] as usize] = Some(ID::DataType(DataType::Sampler)),
				Instruction::OpTypeSampledImage => id_values[data[1] as usize] = Some({
													if let Some(ID::DataType(DataType::Image { dimension, pixel_format, texture_format })) = &id_values[data[2] as usize] {
														ID::DataType(DataType::ImageSampler { 
															dimension: *dimension,
		...
		for (id, x) in id_values.iter().enumerate() {
			match x {
				...
				Some(ID::Variable(data_type, StorageClass::UniformConstant)) => {
					let decorator = &decorators[id];
					if decorator.is_none() { () }
					else {
